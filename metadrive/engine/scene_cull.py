...
class SceneCull:
    ...
    def all_out_of_bounding_box(bounding_box, poses, margin_distance):
        for pos in poses:
            if (
                bounding_box[0] - margin_distance < pos[0] < bounding_box[1] + margin_distance
                and bounding_box[2] - margin_distance < pos[1] < bounding_box[3] + margin_distance
            ):
                return False
        return True
    ...
    def out_of_bounding_box(bounding_box, pose, margin_distance):
        bb2 = [
            pose[0] - margin_distance,
            pose[0] + margin_distance,
            pose[1] - margin_distance,
            pose[1] + margin_distance,
        ]
        if bounding_box[0] > bb2[1] or bounding_box[1] < bb2[0] or bounding_box[2] > bb2[3] or bounding_box[3] < bb2[2]:
            return True
        else:
            return False
